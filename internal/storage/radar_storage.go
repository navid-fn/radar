...
type Storage interface {
	...
	// CreateCommissions inserts batch of Commission into the database
	CreateCommissions(ctx context.Context, commission []*models.Commission) error
...
// CreateCommissions inserts commission using ClickHouse batch insert.
func (s *clickhouseStorage) CreateCommissions(ctx context.Context, commissions []*models.Commission) error {
	...
	batch, err := s.conn.PrepareBatch(ctx, `
		...
		symbol, source, taker, maker, inserted_at
					)
	`)
	if err != nil {
		return err
